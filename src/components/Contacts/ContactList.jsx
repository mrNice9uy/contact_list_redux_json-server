...
const ContactList = props => {
	...
	useEffect(() => {
		if (!isEmpty(contactList)) {
			setFilteredData(
				contactList.filter(contact => contact.name.toLowerCase().includes(search.toLowerCase())),
			);
		}
	}, [contactList, search]);
	...
	const renderActionBtns = useCallback(
		item => {
			return (
				<>
					<Button key={item.id} icon={<EditOutlined />} onClick={() => onEditContact(item)}>
						Edit
					</Button>
					<Popconfirm
						title="Are you sure?"
	...
	return (
		<>
			...
			<div className={classes.searchBarWrapper}>
				<Button disabled={isEmpty(user)} onClick={() => setModalVisibility(true)}>
					Add Contact
				</Button>
				<Search
					allowClear
			...
			<Spinner spinning={isConnectionInProgress}>
				...
				<Table rowKey="id" columns={CONTACTS_COLUMNS} dataSource={renderData(filteredData)} />
			</Spinner>
		</>
	);
};
