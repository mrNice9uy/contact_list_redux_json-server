...
const Profile = props => {
	...
	const getUserInfo = useCallback(async () => {
		try {
			const response = await axios.get(`http://localhost:3006/usersinfo/${userState?.user?.id}`);
			form.setFieldsValue(pick(userState?.user, ['name', 'email']));
			form.setFieldsValue(pick(response?.data, ['lastName', 'website', 'introduction']));
		} catch (err) {
			if (!err?.response) {
	...
	return (
		<Spinner spinning={isConnectionInProgress}>
			...
			<Divider />
			<Form {...PROFILE_LAYOUT} form={form} name="nest-messages" onFinish={onFinish}>
